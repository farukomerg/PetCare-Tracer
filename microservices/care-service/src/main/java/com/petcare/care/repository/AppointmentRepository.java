...
public class AppointmentRepository {
    ...
    private Appointment mapRow(ResultSet rs, int rn) throws SQLException {
        return new Appointment(
                rs.getLong("appointment_id"), rs.getLong("pet_id"),
    ...
    public Long save(CreateAppointmentRequest r) {
        ...
        jdbc.update(con -> {
            PreparedStatement ps = con.prepareStatement(
                    "INSERT INTO appointments(pet_id,vet_name,clinic_name,appointment_time,status,note) VALUES(?,?,?,?,?,?)",
                    new String[]{"appointment_id"});
            ps.setLong(1, r.petId());
            ps.setString(2, r.vetName());
            ps.setString(3, r.clinicName());
            ps.setTimestamp(4, Timestamp.valueOf(r.appointmentTime()));
            ps.setString(5, r.status() != null ? r.status() : "PLANNED");
            ps.setString(6, r.note());
            return ps;
        }, kh);
